...
async function loadPackages() {
  try { packages.value = await get('/packages', { tenantId: store.tenantId }) } catch {}
}
loadPackages()
</script>
  ...
  <div class="portal-container">
    <div class="portal-card">
      ...
      <div v-if="packages.length > 0" style="margin-bottom:20px">
        ...
        <div v-for="item in packages.slice(0, 3)" :key="item.id" class="package-card"
      ...
      <el-button class="portal-full-btn" type="primary" @click="router.push('/login')">账号密码登录</el-button>

      <div class="portal-divider"><span>其他方式</span></div>

      <div style="display:flex;gap:12px">
        <el-button style="flex:1" @click="router.push('/whatsapp')">💬 WhatsApp验证码</el-button>
        <el-button style="flex:1" @click="router.push('/card')">🎫 充值卡</el-button>
      </div>

      <div style="text-align:center;margin-top:20px">
        <el-button link type="primary" @click="router.push('/my')">已有账号？查看个人中心</el-button>
      </div>

      <el-divider style="margin:16px 0" />
      <div style="text-align:center"><a href="/" style="text-decoration:none"><el-button link type="info" size="small">🔧 管理员入口</el-button></a></div>
    </div>
  </div>
</template>
