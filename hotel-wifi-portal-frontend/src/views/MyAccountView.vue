...
const paymentStore = ['wechat', 'alipay'] as const
...
async function handleRecharge() {
  try {
    await post('/payment/wechat/create', {
      tenantId: searchForm.tenantId, memberId: memberInfo.value?.id,
      amount: rechargeAmount.value, packageId: 0,
    })
      ...
      <div v-if="!memberInfo">
        <el-form label-position="top">
          <el-form-item label="租户ID">
            <el-input-number v-model="searchForm.tenantId" :min="1" style="width:100%" size="large" />
          </el-form-item>
          <el-form-item label="手机号">
            <el-input v-model="searchForm.phone" placeholder="请输入注册手机号" size="large" />
          </el-form-item>
          <el-button class="portal-full-btn" type="primary" :loading="loading" @click="searchByPhone">
            查询
          </el-button>
        </el-form>
      </div>
      ...
      <div v-else>
        <el-descriptions :column="1" border size="large">
          ...
          <el-descriptions-item label="账户余额">
            <span style="font-size:20px;font-weight:700;color:#67C23A">¥{{ memberInfo.balance || 0 }}</span>
          </el-descriptions-item>
          <el-descriptions-item label="套餐到期">
            {{ memberInfo.expireAt || '无' }}
          </el-descriptions-item>
          <el-descriptions-item label="状态">
            <el-tag :type="memberInfo.status === 1 ? 'success' : 'danger'">
              {{ memberInfo.status === 1 ? '正常' : '已停用' }}
            </el-tag>
          </el-descriptions-item>
        </el-descriptions>
        ...
        <div style="text-align:center;margin-top:12px">
          <el-button link @click="memberInfo = null">← 切换账号</el-button>
        </div>
      </div>
    ...
    <el-dialog v-model="rechargeVisible" title="在线充值" width="90%" style="max-width:380px;border-radius:12px">
      <div style="text-align:center;padding:10px 0">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin-bottom:16px">
          <el-button
            v-for="n in [10,30,50,100,200,500]"
            :key="n"
            :type="rechargeAmount === n ? 'primary' : ''"
            @click="rechargeAmount = n"
            size="large"
          >¥{{ n }}</el-button>
        </div>
        <el-form-item>
          <el-input-number v-model="rechargeAmount" :min="1" style="width:100%" size="large" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleRecharge">确认支付 ¥{{ rechargeAmount }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
